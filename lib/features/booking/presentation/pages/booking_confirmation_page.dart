...
class _WorkerAssignedView extends StatelessWidget {
  ...
  const _WorkerAssignedView(
      {required this.booking, required this.workerId, required this.status});
  ...
  Widget build(BuildContext context) {
    return FutureBuilder<DocumentSnapshot<Map<String, dynamic>>>(
      ...
      builder: (ctx, snap) {
        ...
        final workerName =
            workerData?['fullName'] as String? ?? 'Your Worker';
        final workerPhone =
            workerData?['mobileNumber'] as String? ?? '';
        final rating =
            (workerData?['rating'] as num?)?.toDouble() ?? 4.0;
        final totalJobs = workerData?['totalJobs'] as int? ?? 0;
        final tier =
            workerData?['verificationTier'] as String? ?? 'green';
        ...
        return SingleChildScrollView(
          ...
          child: Column(children: [
            ...
            Container(
              ...
              child: Column(children: [
                _SummaryRow(Icons.home_repair_service,
                ...
                _SummaryRow(Icons.attach_money, 'Estimated',
  ...
  Widget build(BuildContext context) {
    ...
    return Container(
      ...
      child: Text(tier.toUpperCase(),
          style: TextStyle(fontSize: 9, fontWeight: FontWeight.bold, color: _color)),
    );
  }
}
...
class _StatusRow extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final (icon, label, color) = switch (status) {
      'workerAssigned' => (Icons.directions_run, 'Worker on the way', Colors.orange),
      'workerEnRoute'  => (Icons.directions_run, 'En route to you', Colors.orange),
      'workerArrived'  => (Icons.location_on, 'Worker has arrived', AppTheme.primaryColor),
      'inProgress'     => (Icons.play_circle_outline, 'Service in progress', AppTheme.successColor),
