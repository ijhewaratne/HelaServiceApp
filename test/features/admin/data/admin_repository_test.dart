...
void main() {
  ...
  group('rejectBlueTierUpgrade', () {
    test(
      'marks the worker verificationStatus as rejected, not approved',
      () async {
        ...
        expect(
          verificationDoc.data()?['rejectionReason'],
          'Reference gave negative feedback',
        );
  ...
  group('getPendingWorkers', () {
    test(
      'returns only workers pending review or with submitted documents',
      () async {
        ...
        expect(result.isRight(), isTrue);
        result.fold((_) => fail('Expected a list of pending workers'), (
          workers,
        ) {
  ...
  group('getOpenIncidents', () {
    test(
      'maps Firestore documents to Incident entities and excludes resolved',
      () async {
        ...
        expect(result.isRight(), isTrue);
        result.fold((_) => fail('Expected a list of open incidents'), (
          incidents,
        ) {
  ...
  group('updateIncidentStatus', () {
    test(
      'setting status to resolved records resolvedAt and resolution',
      () async {
  ...
  group('assignWorkerToBooking', () {
    test(
      'updates the booking and mirrors assignment onto job_requests',
      () async {
        ...
        final result = await repository.assignWorkerToBooking(
          'booking_1',
          'worker_5',
        );
        ...
        final jobRequestDoc = await firestore
