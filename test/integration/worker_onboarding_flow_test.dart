...
void main() {
  ...
  group('Worker onboarding repository integration', () {
    test('submitApplication persists application and returns generated id', () async {
      ...
      final created = result.getOrElse(() => throw StateError('Expected Right'));
    ...
    test('updateLocation writes to workers and worker_locations collections', () async {
      await firestore.collection('workers').doc('worker-1').set({
      ...
      final workerDoc = await firestore.collection('workers').doc('worker-1').get();
      final locationDoc =
          await firestore.collection('worker_locations').doc('worker-1').get();
